...
20. 거대한 클래스 (Large Class)
    - 클래스가 할 일이 많아져서 필드와 중복 코드가 늘어나는 것 => 세부 기능 분리하기
    1. 슈퍼클래스 추출하기 (Extract Superclass)
        - 두 클래스의 공통 부분을 슈퍼 클래스로 추출
        - 메소드 추출하기 => 클래스 추출하기(상속) => 필드/메소드 올리기 => 위임
    
21. 서로 다른 인터페이스의 대안 클래스들 (Alternative Classes with Different Interfaces)
    - 비슷한 기능을 지원하는 클래스지만 호환되지 않는 인터페이스를 사용하는 경우
    - 함수명을 맞춰주거나 불가능한 경우 한 단계 더 추상화하기

22. 데이터 클래스 (Data Class)
    - 데이터 클래스 : 메소드 없이 필드+getter/setter만 있는 클래스
    - 필드를 적절한 위치로 옮겨야함
